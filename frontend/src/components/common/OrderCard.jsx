...
export default function OrderCard({ order, footer, meta }) {
        ...
        {order.items?.map((item) => (
          <li key={item.id} className="flex justify-between py-1.5 text-sm">
            {/* <span className="text-ink/70">
              {item.quantity} × Item{item.menu_id}
            </span> */}
           <div className="flex items-center gap-2">
  {/* Quantity Badge */}
  ...
  <span className="font-medium text-ink">
   Item 
            ...
            <span className="price-tag text-ink/80">{formatCurrency(item.price * item.quantity)}</span>
          </li>
        ))}
      </ul>
      ...
      {order.notes && <p className="text-xs text-ink/50 mt-1 italic">"{order.notes}"</p>}

      <div className="flex items-center justify-between mt-4 pt-3 border-t border-sand">
        <span className="text-sm text-ink/60">Total</span>
        <span className="price-tag text-mango-dark font-bold text-lg">{formatCurrency(order.total_amount)}</span>
      </div>
